...
interface TreeStoreState {
  ...
  addNodeFromCatalog: (masterNodeId: string, stageId?: string) => void
  ...
  cancelConnect: () => void
  requestConnection: (sourceId: string, targetId: string) => void
...
export const useTreeStore = create<TreeStoreState>((set, get) => ({
  ...
  addNodeFromCatalog: (masterNodeId, stageId) => {
    const state = get()
    ...
    const newNode: TreeNode = {
      ...
      position: { x: nodeXForStage(stage.order), y: defaultNodeY(siblings.length) },
      editorState: { priority: master.priority, status: master.status },
    }
  ...
  requestConnection: (sourceId, targetId) => {
    ...
    set({ pendingConnection: { sourceId, targetId } })
  },
